...
from modules.analytics import compute_kpi, compute_all_stats, get_full_answers
from modules.charts import (
    render_block_charts, render_chart,
    render_pie, render_table,
)
...
from config import BLOCK_NAMES, QUESTIONS
    ...
    with st.expander(f"Блок {block_num}. {block_name}", expanded=(block_num == 1)):
        for col, cfg in QUESTIONS.items():
            ...
            if global_chart_type == "Столбчатая":
                ...
                if fig is not None:
                    st.plotly_chart(fig, use_container_width=True, key=f"b{block_num}_{col}")
            elif global_chart_type == "Круговая":
                fig = render_pie(df_filtered, col, cfg["question"])
                if fig is not None:
                    st.plotly_chart(fig, use_container_width=True, key=f"pie_{block_num}_{col}")
            elif global_chart_type == "Таблица":
                table = render_table(df_filtered, col, cfg["question"])
                if table is not None:
                    st.dataframe(table, use_container_width=True, hide_index=True)

        # Текстовые колонки — топ слов + ответы
        for col, cfg in QUESTIONS.items():
            ...
            fig = render_chart(df_filtered, col, cfg)
            if fig is not None:
                st.plotly_chart(fig, use_container_width=True, key=f"text_{block_num}_{col}")

            answers = get_full_answers(df_filtered[col])
            if answers:
                st.caption("Ответы респондентов:")
                for a in answers:
                    st.markdown(f"- {a}")
